    ...
    var body: some View {
        ZStack(alignment: .topTrailing) {
            ...
            Button {
                ...
                Image(systemName: "plus").padding(10).foregroundColor(.white).background(.black).cornerRadius(50).padding()
            }
...
struct ProductCart_Previews: PreviewProvider {
    static var previews: some View {
        ProductCart(product: productList[0]).environmentObject(CartManager())
    }
}
